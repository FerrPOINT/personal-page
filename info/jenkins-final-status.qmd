# Статус настройки Jenkins CI/CD

## ✅ Выполнено автоматически

### 1. Сервер деплоя настроен
- **Хост**: `7eb10d5af2ad.vps.myjino.ru:49233`
- **Пользователь**: `root`
- **Путь**: `/opt/personal-page`
- **Репозиторий**: обновлен до последней версии
- **Docker**: установлен и работает
- **Docker Compose**: установлен и работает
- **Git**: установлен и работает

### 2. Jenkinsfile обновлен
- Использует реальный хост сервера
- Правильный SSH порт (49233)
- Скрипт деплоя: `scripts/deploy.sh`

### 3. SSH ключ настроен
- Публичный ключ добавлен на сервер
- Приватный ключ готов для Jenkins credentials

## ⚠️ Требуется ручное действие

### Создание SSH Credentials в Jenkins

1. Откройте Jenkins: http://192.168.1.49:32768/
2. Перейдите: **Manage Jenkins** → **Credentials** → **System** → **Global credentials (unrestricted)**
3. Нажмите **Add Credentials**

4. Заполните форму:
   - **Kind**: `SSH Username with private key`
   - **Scope**: `Global`
   - **ID**: `jenkins-ssh-deploy-key` (обязательно!)
   - **Description**: `SSH key for deployment server`
   - **Username**: `root`
   - **Private Key**: `Enter directly`
   - Вставьте приватный ключ (см. ниже)

5. Нажмите **OK**

### Приватный ключ для credentials

```bash
# Получить ключ:
cat ~/.ssh/id_ed25519
```

Или используйте скрипт:
```bash
bash scripts/create-ssh-credentials-manual.sh
```

## 🚀 После настройки credentials

Запустите тестовый build:

```bash
bash scripts/trigger-jenkins-build.sh
```

Или через Jenkins UI:
- http://192.168.1.49:32768/job/personal-page-deploy/
- Нажмите **Build Now**

## 📊 Проверка статуса

```bash
# Проверить статус build:
curl -s -u ferrpoint:YOUR_JENKINS_API_TOKEN \
  "http://192.168.1.49:32768/job/personal-page-deploy/lastBuild/api/json?tree=number,result,building"

# Посмотреть логи:
curl -s -u ferrpoint:YOUR_JENKINS_API_TOKEN \
  "http://192.168.1.49:32768/job/personal-page-deploy/lastBuild/consoleText" | tail -50
```

## 🔍 Архитектура деплоя

```mermaid
graph LR
    A[GitHub Push] --> B[Jenkins Webhook]
    B --> C[Jenkins Pipeline]
    C --> D[Checkout Code]
    D --> E[SSH to Server]
    E --> F[Run deploy.sh]
    F --> G[Docker Compose]
    G --> H[Application Running]
```

## 📝 Конфигурация

### Jenkinsfile
- **DEPLOY_HOST**: `7eb10d5af2ad.vps.myjino.ru`
- **DEPLOY_PORT**: `49233`
- **DEPLOY_USER**: `root`
- **DEPLOY_PATH**: `/opt/personal-page`
- **Credentials ID**: `jenkins-ssh-deploy-key`

### Скрипт деплоя
- **Файл**: `scripts/deploy.sh`
- **Расположение**: на сервере в `/opt/personal-page/scripts/deploy.sh`
- **Действия**:
  1. Обновление кода из Git
  2. Копирование env.prod в .env
  3. Остановка старых контейнеров
  4. Сборка и запуск новых контейнеров
  5. Запуск миграций БД
  6. Проверка здоровья сервисов

## ✅ Готовность к работе

После создания credentials в Jenkins, pipeline будет полностью автоматизирован:

1. Push в GitHub → автоматический trigger
2. Jenkins клонирует репозиторий
3. Jenkins подключается к серверу по SSH
4. На сервере выполняется `deploy.sh`
5. Приложение обновляется и перезапускается

