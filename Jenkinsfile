            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        // Установка браузеров Playwright (если еще не установлены)
                        sh 'npx playwright install --with-deps chromium || echo "⚠️  Playwright браузеры уже установлены"'
                        
                        // Запуск критичных тестов (LanguageSwitcher и Contact Form)
                        sh '''
                        ...
                        // Не блокируем деплой, если тесты провалились (можно изменить на exit 1 для блокировки)
