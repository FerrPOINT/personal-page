...
- **Jenkinsfile** в корне проекта определяет pipeline для автоматического деплоя
- Pipeline автоматически запускается при push в репозиторий
- Выполняет сборку Docker образов и деплой на сервер

**Документация:**
- **Быстрый старт**: `info/jenkins-quickstart.qmd`
- **Полная документация**: `info/jenkins-pipeline.qmd`
- **📚 Руководство для других проектов**: `info/jenkins-cicd-setup-guide.qmd` - полное пошаговое руководство по настройке CI/CD для любого проекта
- **⚡ Быстрая шпаргалка**: `info/jenkins-quick-reference.qmd` - краткая справка для быстрой настройки
