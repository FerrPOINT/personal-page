...
ifeq ($(OS),Windows_NT)
	@if exist env.local ( \
		copy /Y env.local .env >nul 2>&1 && echo ✅ env.local скопирован в .env \
	) else ( \
		echo ❌ Файл env.local не найден && echo 💡 Создайте файл env.local с переменными окружения && exit /b 1 \
	)
else
	@if [ -f env.local ]; then \
...
ifeq ($(OS),Windows_NT)
	@if exist env.prod ( \
		copy /Y env.prod .env >nul 2>&1 && echo ✅ env.prod скопирован в .env \
	) else ( \
		echo ❌ Файл env.prod не найден && echo 💡 Создайте файл env.prod с переменными окружения && exit /b 1 \
	)
else
	@if [ -f env.prod ]; then \
